...
app.get('/api/location/area', async (req, res) => {
  const { centerLat, centerLng, radiusMeters, deviceId } = req.query;

  if (!centerLat || !centerLng || !radiusMeters) {
    return res.status(400).json({ 
      message: 'Los parámetros centerLat, centerLng y radiusMeters son requeridos' 
    });
  }
  ...
  try {
    ...
    // Formula de Haversine para calcular distancia en PostgreSQL
    // Convertir radio de metros a grados (aproximado)
    const radiusDegrees = radius / 111320; // 1 grado ≈ 111.32 km
    ...
    const values = [lat, lng, radiusDegrees];
    ...
    // Filter by exact circular distance
    const filteredResults = result.rows.filter(row => 
      row.distance_meters <= radius
    );
