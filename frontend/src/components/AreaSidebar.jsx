...
const AreaSidebar = ({ users, selectedDevices, onDeviceToggle, areaInfo, onShowRoutes }) => {
            ...
            filteredUsers.map((user) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Button to view routes - only visible when selected */}
                  {isSelected && onShowRoutes && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        onShowRoutes(user.id);
                      }}
                      className="w-full mt-2 px-3 py-2 bg-white/10 hover:bg-white/20 rounded-lg text-white text-xs font-medium transition-all duration-200 flex items-center justify-center gap-2"
                    >
